...
def itertranspose(it: Iterable[T],
                  /,
    ...
    if not all(isinstance(a, (str, Callable)) for a in applicatives):
        raise ValueError(f'`applicatives` should be a string (attribute name) or a one-argument '
    ...
    if not all(isinstance(f, Callable) for f in filters):
        raise ValueError(f'`filters` should be one-argument callables:\n{applicatives=}')
