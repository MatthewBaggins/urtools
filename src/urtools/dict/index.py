...
from typing import Any, List, Hashable, Iterable, Literal, TypeVar, Union, cast

from urtools.list.list_nans import list_has_nans
from urtools.dict.join_dicts import join_dicts
from urtools.type.type_guards import is_iter
...
class KeysAndNegKeysAreNoneError(Exception):
    message: str = field(init=False)
    def __post_init__(self):
        self.message = 'Either `keys` or `neg_keys` must be `None` but here both are `None`.'
